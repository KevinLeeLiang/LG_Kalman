...
class KalmanFilter2Order:
    ...
    def __init__(self, size=100, sampleTime=0.05, q11=0.001, q12=0.0, q21=0.0, q22=0.1, r11=0.001, r12=0.0, r21=0.0,
        ...
        #  参数: 噪声方差矩阵——可调参数 #########################################################################
        self.T = sampleTime
        self.A = np.matrix([[1, sampleTime], [0, 1]])  # 状态矩阵,
        self.H = np.matrix([[1, 0], [0, 1]])  # 观测矩阵,
        self.Q = np.matrix([[q11, q12], [q21, q22]])  # 过程噪声的协方差矩阵：两个状态值x=[x1 ; x2]，两个状态值的过程噪声分别为w=[w1; w2];
        #  过程噪声符合高斯分布p(w1)~(0,q11), p(w2)~(0,q22); 两个状态值的过程噪声组成的协方差矩阵为Q,  p(w)~(0,Q)；Q=[q11,q12; q21,q22 ]
        self.R = np.matrix([[r11, r12], [r21, r22]])  # 测量噪声的协方差矩阵；两个传感器的测量值z=[z1 ; z2]，两个传感器的测量噪声分别为v=[v1; v2];
        #  测量噪声符合高斯分布p(v1)~(0,r11), p(v2)~(0,r22); 两个状态值的测量噪声组成的协方差矩阵为R,  p(v)~(0,R)；R=[r11,r12; r21,r22 ]
        self.Ieye = np.matrix([[1, 0], [0, 1]])
        #  2I2O-初始化各矩阵 #########################################################################
        self.filterValue = np.matrix([0, 0]).T  # 校正：后验估计值（卡尔曼滤波结果）；2x1
        self.kalmanGain = np.matrix([[1, 0], [0, 1]])  # 校正：卡尔曼增益；
        self.P = np.matrix([[1, 0], [0, 1]])  # 校正：更新误差协方差；初始值不重要，都赋值为1就可以
        self.P_pre = np.matrix([[1, 0], [0, 1]])  # 预测：先验误差协方差值；初始值不重要，都赋值为1就可以

        #  其它变量 #########################################################################
        self.measurementLast = np.matrix([0, 0]).T  # 校正：后验估计值（卡尔曼滤波结果）；2x1
        self.measurementNew = np.matrix([0, 0]).T  # 校正：后验估计值（卡尔曼滤波结果）；2x1
        self.Counter = 0
    ...
    def kalmanFilterCalc(self, new_speed=np.zeros([3]), new_acc=np.zeros([3])):
        ...
        for i in range(self.dataSize):  # self.dataSize =5;    0,1,2,3,4
            if 0 == i:  # 第一次进来初始化
                ...
                self.filterValue = np.matrix([[speed1], [acc1]])
            ...
            if acc_delta > limitAccDelta:
                ...
                accModify = acc_KF_last + limitAccDelta
            self.measurementNew = np.matrix([[speedModify], [accModify]])
